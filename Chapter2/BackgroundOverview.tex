...
Moreover we give an overview of the core problem
of the research \textit{PureCircuit}, and its connections
with search problems and \textit{Kleene logic}. % TODO: Cleanup 
...
where we can represent functions or relations as explained
in the definitions \ref{def:search-problems}.
In addition, we reference an additional subclass of these
problems known as \textit{Total Search problems}, in
which for every input, there must exist at least one solution.
...
Various \textbf{TFNP} were introduced by various researchers which form the hierarchy as seen in \ref{fig:chap-2:tfnp-hierarchy}
...
which takes advantage of the combinatorial fact that if there exists an unbalanced node in a directed graph, %% TODO: Cleanup
...
Alexandros et al. in the paper \cite{hollender_ComplexityMultisourceVariants_2018} was able to demonstrate that for several modifications
of the original problem such as adding
...
with the problems such as \textit{SourceOrExcess}$(k,1)$ where $k$ it the maximum in-degree for each node
as defined in \ref{def:source-or-excess}.
All these problems remain \textbf{PPAD} complete. More interestingly, works by Papadimitriou \cite{papadimitriou_GraphtheoreticLemmataComplexity_1990},
Beame et al. \cite{beame_RelativeComplexityNP_1998} and
Alexandros et al. \cite{hollender_ComplexityMultisourceVariants_2018} were able to demonstrate that in the more general case of superpolynomial graphs,
...
%:TODO: work `SourceOrExcess`. You probably want to define its full thing in the main findings section -3
...
We will mainly look at two of these problems, known as \textit{RLeafD} and \textit{2D-EOL}
...
Chen et al. \cite{chen_Complexity2DDiscrete_2009} introduced a reduction
on how one can parsimoniously reduce an \textit{EndOfLine} problem to a \textit{2D-EOL} problem
(written as \textit{RLeafD} in their paper)
using planar graph embeddings. We find this construction useful for our results
and therefore we give a full report on how it works.



\paragraph*{Multi-Source variants}
Lastly we introduce the class of variants for which the \textit{EndOfLine} problem
which we extend the known number of sources.
These
...
in some hypercube $[N]^d$, we assigned $d$ colours, where for colour $i$ have choices
...
to demonstrated \textbf{PPAD}-hardness of many problems.
...
It is important to observe that the \textit{StrongSperner} problem is defined for hypercubes
of dimensionality $n$ and width $m \in n^{O(1)}$.
...
We will use as reference the \textbf{PPAD}-complete \textit{1-MC} problem. We refer
...
\subsection{Bridging Counting Complexity, Search problems and Kleene Logic}
