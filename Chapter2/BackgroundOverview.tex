...
as shown in figure \ref{..}. These problems were introduced by various researchers as seen in {}, % TODO: Here
...
\begin{definitionbox}{\scn{2D-EOL} problem \cite{chen_Complexity2DDiscrete_2009,hollender_ComplexityMultisourceVariants_2018}}{2d-eol}
    \label{2d-eol}
...
\begin{corollarybox}{K-bit Hazard Construction \cite{ikenmeyer_ComplexityHazardfreeCircuits_2019}}{k-bit-haz}
    ...
    such that:
    \begin{align*}
        \textit{size}(C')\quad & = \left(\frac{ne}{k}\right)^k (\textit{size}(C) + 6) + O(n^{2.71k}) \\
        \textit{depth}(C')\quad & = \textit{depth}(C) + 8k + O(k \log n)
    \end{align*}
