...
\begin{figure}
    ...
    \caption[Parsimonious Reduction between Sudoku and graph colouring]{Example of reducing a 6x6 sudoku puzzle, into a graph colouring problem}
    \label{fig:chap-2:pars-reduction}
\end{figure}

%% TODO: Reminder to check before submission if needed. Chris 22/Aug says not
...
\begin{definitionbox}{Kleene value ordering \cite{mukaidono_BternaryLogicFunction_1972}}{kleene-order}
    The \textit{instability ordering} or \textit{information ordering} for $\leq^u$ is defined as: $\bot \leq^u 0,1$. The $n$-dimensional
...
\begin{definitionbox}{Natural functions \cite{ikenmeyer_ComplexityHazardfreeCircuits_2019}}{nat-func}
    ...
    \begin{enumerate}
        ...
        \item \textit{Preserves monotonicity}: $\forall x,y \in \mathbb{T}^n: x \preceq y \implies F(x) \preceq F(y)$ \ref{def:mono-func} \ref{def:kleene-res}
    \end{enumerate}
\end{definitionbox}
...
resolutions of $0\bot$ could be either $00$ or $01$.
...
it is an \textbf{NP}-hard problem \cite{ikenmeyer_ComplexityHazardfreeCircuits_2019}.
...
\begin{figure}[h!]
    ...
    \caption[Example of hazard and non-hazard-free circuits]{Hazard circuit and hazard-free circuit. Figure by \cite{ikenmeyer_ComplexityHazardfreeCircuits_2019}}\label{fig:hazard-example}
\end{figure}
...
These have been introduced by Chandra et al. \cite{chandra1981alternation} and the idea is that we introduce universal and existential quantifiers
...
one impose the condition that all subprocesses $\{\beta_i\}_{i \in [k]}$ must accept accept or in case of a negation state, if $\beta_1$ accepts, then
...
We now say that a labelling of configurations is a map $\ell: C_M \to \{0, \bot ,1\}$.
...
where they reduce $\textit{\#CircuitSAT} \subseteq ((\textit{\#EndOfLine}) -1) /2$ by creating a graph, where
for each input $x$, we create a source and sink pair $(0x, 1x)$ if and only if $f(x) = 1$.
...
arbitrary oracles. They demonstrated that for several problems in \textbf{TFNP} as seen in figure \ref{fig:chap-2:tfnp-hierarchy},
...
problems with no combinatorial interpretations. Our last objective is the overall
...
manage to yield any positive results, we provides several tools, new problems and
conjectures that may help in future research.
...
We therefore provide this additional list of software deliverables:
\begin{enumerate}
    ...
    \item Establish algorithms to find solutions or at least approxiamte solutions.
    \item Establish algorithms to enumerate solutions for small graphs.
\end{enumerate}
