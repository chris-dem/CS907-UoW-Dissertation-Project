...
The current project includes a software element, in order to aid with the gadget development.
We provide three core requirements as enumerated in the following list:
\begin{enumerate}
    \item Visualise and instance of a pure circuit and check for correctness.
    \item Given a \textit{PureCircuit} graph, find a satisfying assignment.
    \item Given a \textit{PureCircuit} graph, enumerate through all possible assignments.
\end{enumerate}
We split these section into the following components:
Tooling selection and development architecture, Visualisation tooling,
Solution Finding and Solution Enumeration.

\subsection{Tooling Selection}
Using figure {..} as reference, we outline the main choice for our tools

\paragraph*{Rust Programming Language} The development language used for is \texttt{Rust}.
...
\paragraph*{Bevy and the ECS architecture} \texttt{Bevy} is a \texttt{Rust} library,
...
figure \ref{fig:chap-4:ecs-example}. The core
...
tool and editor-like vibe where the user can move the screen and individual
nodes themselves as well as interact with them. On the other hand,
...
\paragraph*{Software Testing methodology}
...
It works by testing our function, across a series of possible inputs. If a failed condition
is raised, then it uses \textit{Shrinking} strategies in order to find the smallest assignment that fails
the test. %% TODO: Consider adding a figure


\subsection{Program Visualisation Summary}
...
in order to keep the computational expenses low. Each time we modify the stucture of the circuit, we reset the solution set.
We emphasize that these buttons are disabled if the circuit contains any invalid arities.
...
Overall this algorihtm, although it runs the fastest out of all the others, it
may significantly struggle against large graphs or instances
that multiple changes need to occur in order to reach an optimal point.
Future ideas can adopt \textit{Simulating annealing} which %% TODO add refernce
...
stricter and stricter with the next neighbour choice.

\subsubsection{Evolutionary Algorithm}

Evolutionary algorithms as the names suggests are based of the
idea of evolution. 
%% TODO: Add reference as to how they started
A simulation step of an evolutionary algorithm can be summarised in figure {..}, 
where we essentialy have 5 core elements:
\begin{enumerate}
    ...
    In literature, there are many types that one can use %% TODO add reference please
    such as \textit{Roulette based} selection, or \textit{Tournament based} selection. 
    We leave several choices available for the user to choose.
    ...
    \textbf{Crossover step}, stochastially modify each gene in
    order to introduce diversity.
\end{enumerate}
...
\begin{breakablealgorithm}[h!]
    ...
    \begin{algorithmic}
        \Function{BacktrackInstatiation}{pc}
        \State $V \leftarrow \textit{PCExtract}(pc)$
        \State $S \leftarrow \textit{Map}(V, \texttt{KleeneSet::all}())$
        \Comment {Create a state vector that contains the \textit{KleeneSet} data-structure.}
        \State $Q \leftarrow$ \Call{PriorityQueueInit}{()}
        \For{$\textit{el} \in V$}
        \State \Call{PushPQ}{Q, \texttt{el.indesx}, \Call{KeyStruct}(3, el)}
        \EndFor
        \State \Return $S, Q$
        \EndFunction

        \Function{BacktrackPureCircuit}{pc \texttt{:PureCircuit}}
        ...
        \Function{BacktrackIteration}{$\text{pc}, A, S, Q$}
        \If{\Call{IsEmpty}{$Q$}}
        \State \Return $[A]$
        \EndIf
        \State $m \leftarrow$ \Call{ExtractMin}{$Q$}
        \State $r \leftarrow []$
        \For{$v \in S[m]$}
        \State $S' \leftarrow$ \Call{Clone}{$S$}
        \State $Q' \leftarrow$ \Call{Clone}{$Q$}
        \State $\textit{result} \leftarrow$\Call{UnitPropagate}{$\text{pc}, m, v, A, S', Q'$}
        \If{$\textit{result} = \bot$}
        ...
        \Function{UnitPropagation}{$\text{pc},m , v, A, S, Q$}
        \State \Call{AssignNode}{}
        \State \Call{PropagateNode}{}
        \While{\texttt{Q.is\_empty()}$\wedge$\texttt{Q.peek().key.value\_len}$=1$}
        \State $\text{ind} \leftarrow$\Call{Pop}{$Q$}
        \State $v \leftarrow$\Call{Pop}{$S[\text{ind}]$}
        \State \Call{AssignNode}{}
        \State \Call{PropagateNode}{}
        \EndWhile
        \If {\texttt{Q.is\_empty}()}
        \State \Return $\top$
        \EndIf
        \If {\texttt{Q.peek.key.value\_len}$=0$}
        \State \Return $\bot$
        \EndIf
        \State \Return $\top$
        \EndFunction

        \Function{PropagateNode}{\texttt{pc, m , A, S, Q}}
        \For{$g \in$ \Call{GetAllGateNeighbours}{pc, m}}
        \State $I \leftarrow$ \Call{GetValueSets}{\texttt{Input}}
        \State $O \leftarrow$ \Call{GetValueSets}{\texttt{Output}}
        \State $(I', O') \leftarrow$ \Call{EvaluateSets}{$g, I, O$}
...
and uses as keys the structure \texttt{KeyStruct}$(e)$.
The \texttt{KeyStruct} is a structrue that orders elements lexicographically
based on the following order:
\begin{enumerate}
    \item \textbf{In descending}: The number of remaining elements
...
\end{enumerate}
We argue that the above strategy minimises the recursive calls breadth. Moreover,
nodes with \textit{in-degree} of $0$ as for each value of their assignment, a solution should exist.
Lastly, prioritising nodes with the highest neighbour count implies that we
propagate changes to many gates and therefore prioritise the \textit{Unit Appplication}
procedure.
...
For example, we an \textbf{AND} gate with triplet of values $(\{0,\bot,1\}, \{0,\bot,1\}, \{0,\bot,1\})$.
...
At the tree of every execution call, merge the solution assignments in each level and return
the sets.
...
see that each time, our priority queue will select a node and will recurse through each of each possible values,
creating a call tree with depth of at most $n$. Therefore we can argue even if the running time of the algorithm
is $O(3^n)$, we can observe that the unit propagation and set reduction methods reduce the width and depth greately.
...
that will give a satisfying assignment. If the value set of a node is zero, it means that no assignment
will lead to a solutin and therefore we reject the current path.
...
This implies that our resulting array of assignments contains only valid solutions.
A
...
is to treat weakly directly connected components individaully. Trivially an assignment in one
weakly connected component should not affect the solution set of the other one. 
We can therefore reduce the solution spaces of our problems by finding
...
\begin{enumerate}
    \item Identify the strongly connected components of the circuit, using polynomial
    time algorithms such as Kosaraju's Algorithm or Gabow's Algorithm. %%TODO: ADD citations
    ...
    use the distance from base instead. 
\end{enumerate}
...
Lastly an additional heuristic that we failed to consider is using the notion
of alternation or varying paths as highlighted in {..}. % TODO add references
Essetnialy by continuously "propagating" the gate values, 
one may be able to identify nodes that tent to alternate. This method
was used to identify nodes that should be assingend with the $\bot$ value.
...
for the $\bot$ value, the outputs could be multiple. This is the reason
why we also don't necesserily propagat solution sets and instead only values as
