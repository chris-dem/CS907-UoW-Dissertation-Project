...
\begin{enumerate}
    \item \textbf{Chromosomes}: Solution assignments of our \textit{PureCircuit} instance.
    \item \textbf{Population}: A set of \textbf{chromosomes}.
    \item \textbf{Fitness step}: Evaluation step. We measure how good the assignment is
          by counting the number of errors it has.
    \item \textbf{Selection step}: Given a selection strategy, select
          suitable candidates in order to create the population for the next step.
          ...
          chrosomomes and pick the best one. This process is repeated until enough chromosomes have been selected.
    \item \textbf{Crossover step}: Given two chromosomes from the
...
for many situations, we allow for the user to tune the parameters and strategies to their needs
in order to adapt to each solution.
Moreover, we use speciation techniques were separate our chromosomes
...
work and in the appendix we provide the full table.
...
%     \end{tabular}
%     \caption{\textbf{OR} value propagation rules}
%     \label{tab:chap-4:purify-propagation}
% \end{table}
...
Value that we discard are nodes that guarantee to give unsatisfying assingments, and therefore
