...
We ensure the user is in mouse mode, hold A and press left click to place the node,
as seen in the figures \ref{fig:app:node-addition}.
We observe in figure \ref{fig:chap-5:full-add} that the \textsc{Copy} gate
is circled in pink to denote invalid arity, meaning it is missing its input
or output edge. In addition, hovering over a node
...
We can observe from figure \ref{fig:app:hold-edge} that we highlight the source node
with light blue and the
heterogeneous nodes with orange. If we select a gate, we highlight only
value nodes that have an in-degree of $0$.
Pressing ESC or changing mode can cancel the selection operation.
Selecting any of the orange nodes completes
the edge addition operation as seen in figure \ref{fig:app:some-add}. We can also observe that the \textsc{Copy} has
