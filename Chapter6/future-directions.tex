...
how Kleene logic can be incorpoeated with \textbf{PPAD} problems. For
...
is not clcar by current complexity assumptions and indeed proving \textbf{PPAD}-hardness
...
\textit{HF-nD-Counting-SS} as previously stated and whether they
retain the parsimonious reduction like the \textit{HF-nD--SS} problem.
...
Moreover, given our \textbf{PPAD} hierrchies, we can create the following conjecture
...
$v_i = \bot$. We argue the existance gates which exhibit any behaviour of the sort $f \in [0,1]^k \to [0,1]^m$
such that $f$ is contiuous, where they give may rise to stronger combinatorial expression
than our current set.


\subsection{Software future Directions}

Given the algorithms that we proposed for our software, we argue that there are several improv\-ements
that can be made.
