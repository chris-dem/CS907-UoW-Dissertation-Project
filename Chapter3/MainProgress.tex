...
\begin{proof}
    Given circuit $\lambda: (\{0,1\}^k)^n \to \{-1,+1\}^n$ from \textit{nD-SS}, we create
    $\Lambda: (\{0,1\}^{k+1})^n \to \{-1,+1\}^n$ such that:
    $$
        \Lambda((x_{i})_{i \in [n]}) = \lambda\left( \left( { \left\lfloor \frac{x_{i}+1}{2}  \right\rfloor } \right)_{i \in [n]} \right)
    $$
