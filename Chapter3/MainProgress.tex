...
when working with \textit{PureCircuit}, one cannot fully work with binary logic.
...
\begin{definitionbox}{\textit{StrongSperner} anchor points}{ss-anchor}
    ...
    the $\textbf{Anc}(H)$ as
    $$
        \textbf{Anc}(H) = \min_{p \in H} \|p\|_{\infty}
    $$
...
\begin{definitionbox}{Kleene Unary Numbers}{kleene-unary}
    ...
    $$
        M^k_n \triangleq \bigtimes_{i \in [n]} \{1^j\bot^k \mid \forall j \in \mathbb{N}_0 : j + k \leq M\}
    $$
\end{definitionbox}

% \begin{definitionbox}{Kleene Binary Numbers}{kleene-binary}
%     \label{def:kleene-binary}
%     Given a number $p \in M$, we refer to its binary representation 
%     $$
%         M^k_n \triangleq \{1^j\bot^k \mid \forall j \in \mathbb{N}_0 : j + k \leq M\}^n
%     $$
% \end{definitionbox}

% As we can observe, we can represent a hypercube $n$-dimensional cube
% using either $(p_i)_{i \in [n]} \in M^1_n$ such that
...
\begin{proof}
    ...
    \begin{equation}
        \Lambda((x_{i})_{i \in [n]}) = \lambda\left( \left( { \left\lceil \frac{x_{i}+1}{2}  \right\rceil } \right)_{i \in [n]} \right) \label{eq:chap-3:trans-double}
    \end{equation}
    ...
    \begin{proof}
        ...
        \begin{align*}
            \forall (2p_i)_{i \in [n]} \in (\{0,1\}^{k+1})^n, b \in \{0,1\}^n & :\Lambda((2p_i + b)_{i \in [n]})\,                                                  \\
                                                                              & = \lambda\left(\left(\left\lceil\frac{2p_i + b + 1}{2} \right\rceil\right)_{i \in [n]}\right) \\
                                                                              & = \lambda\left(\left(\left\lceil p_i + \frac{b + 1}{2} \right\rceil\right)_{i \in [n]}\right) \\
                                                                              & = \lambda\left((p_i + b)_{i \in [n]}\right) \\
        \end{align*}
        Which implies that the cube anchored at $p$ must also be panchromatic.
    \end{proof}
    To ensure that $\Lambda$ is hazard free we can use the \ref{col:k-bit-haz} where, we can create a
    $n$-bit hazard free pure circuit. One must observe that even though, the size of the circuit increases exponentially
    with respect to the dimensions, since the problem scales with $k$ and we assume $k \gg n$, the size will scale
    up to a constant with respect to the input size. Therefore, we can parsimoniously find all panchromatic cubes. 
\end{proof}
...
% solutions and therefore can behave unpredictably. 

We will mainly focus on subproblems of the \scn{HF-StrongSperner} where two cubes do not overlap.
