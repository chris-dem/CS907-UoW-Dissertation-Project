...
More specifically, we list the following list of theorems:
\begin{enumerate}
    ...
    \item $\textit{\#EndOfLine} \subseteq \textit{AntipodalStrongSperner}$ %% TODO: Add reference
\end{enumerate}
...
is panchromatic. This means that for the same cube, for the same cube we can have
...
we focus on \textit{Discrete panchromatic cubes}, which are
subsets of \textit{StrongSperner} in which there are no two touching
panchormatic cubes or more formally we use the definition in \ref{def:hf-strong-sperner}.
...
\begin{definitionbox}{\textit{HF-DiscreteStrongSperner} problem}{hf-strong-sperner}
    \label{def:hf-strong-sperner}
    An \textit{HF-StrongSperner} instance $\lambda$ such that $\forall p \in M_1^n$, if $\lambda(p)= \bot^n$, then
    $$
        \forall y \in M_0^n: p \preceq y \implies y \text{ does satisfy the labelling condition \ref{def:bipolar-colouring}}
    $$
...
\begin{definitionbox}{\textit{HF-AntipodalStrongSperner} problem}{hf-strong-sperner}
    An \textit{HF-DiscreteStrongSperner} instance $\lambda$ such that $\forall p \in [M^1]^n$, if $\lambda(p)= \bot^n$,
...
The above claim sense that in a discrete antipodal panchromatic solution,
we expect to find all $2^d$ possible colours.
...
\begin{proof}
    ...
    $$
        ...
        \end{pmatrix}
    $$
    Where we denote $+ = (+, +), -=(-,-), \pm = (+,-)$ and $\mp = (-,+)$. 
    We can observe indeed that the panchromatic square that is antipodal, discrete and contains all the colourings.
...
\begin{proof}
    ...
    \begin{enumerate}
        \item  \textbf{Input nodes}: Vector of $n$ value nodes which will be represented as $x_i$ for element $i$.
        \item  \textbf{Bit generator gadget}: Given a value node $x_i$, apply the \textit{Bit generator} gadget such that we
              create Kleene unary number as defined in \ref{def:kleene-unary}, where $\mathbf{x}[x^i] \in M^{\leq 1}_n$. We will refer
              to the bit generator circuit as $C_i$.
        \item  \textbf{Circuit application gadget}: Since the above numbers form coordinates in a
              $M_{\leq 1}^n$ space, we apply the $\Lambda$ function to extract a set of output nodes $(u^i)$.
        \item  \textbf{Output nodes}: We use the \scn{Copy} gate to copy $u^i \to x_i$, for all $i \in [n]$.
    \end{enumerate}
    ...
    It suffices to show that every vector $\mathbf[x]{x} \in M_1^n$ and $\forall i \in [n]: \mathbf{x}[u^i] = \bot$.
    ...
    By our  purification lemma \ref{clm:chap-3:pur-claim} we know that $\mathbf{x}[x^i] = 0^i$.
...
We use this fasct to add $4$ additional edges
denoted in blue, using the figure \ref{fig:chap-2:rleafd-new-edges} and we add them to $E^2_n$.
...
\textit{2D-EoL} problem. Key insight, is the observation that every planar embedding of a graph of an \textit{EndOfLine}
...
$\forall v\in V_n: \textit{deg}(v) \in \{(1,0), (0,0), (0,1)\}$. We can
define now define the \textit{RLeafD} problem as such

\begin{definitionbox}{\textit{RLeafD} problem \cite{chen_Complexity2DDiscrete_2009,hollender_ComplexityMultisourceVariants_2018}}{rleafd}
    \label{def:rleafd}
    A \textit{2D-EoL}, $S,P$ but instead of defining it in a $2^k \times 2^k$ space, we
    use the grid defined by the graph $G_{2^k}= (V_{2^k}, E_{2^k})$
\end{definitionbox}

Clearly we can observe that by the above connstruction of edge removal, all \textit{EndOfLine} problems
parsimoniously reduce to the \textit{RLeafD} problem.


\paragraph*{Bipolar colouring of \textit{RLeafD}}
\label{par:bipolar-rlaefd}

Given an \textit{RLeafD} we construct space :
$$
...
\begin{theorembox}{}{parsimonious-2d-strong-eol-to-antipodalss}
    $$
        \textbf{\#PPAD}(\textit{2D-StrongRLeafD}) \subseteq \textbf{\#PPAD}(\textit{AntipodalStrongSperner})
    $$
\end{theorembox}
...
\begin{proof}
    ...
    We are given as an input a $2^n \times 2^n$ grid representing any \textit{2D-StrongRLeafD} problem.
    The procedure is summarised as follows:
    Given dimension to fold $i$ where width of $i$ is $n > 8$, apply the \textbf{padding operation} such that $n' = 3 \cdot s + 1$ for some $s \in \mathbb{N}$.
    ...
    \begin{definitionbox}{Snake Embedding operation $S(\Lambda^t, i)$}{snake-emb-op}
        ...
        $\Lambda^{t+1}: M^{t+1} \to \{-1, +1\}^{n+1}$ such that $m'_i = s + 3, n_{t+1} = n_t +1, m_{n_{t+1}} = 7$, where $\Lambda^{t+1}$ is defined
    ...
    \begin{proof}
        ...
        We call the dimension that affects $\Delta_j$  as the effective dimension.
        ...
        $$
        \forall b \in \{0,1\}^{n_{t}}, j \in [n_{t+1}]:  [\lambda(p_{-n_{t+1}}(0))]_j = - [\lambda(p_{-n_{t+1}}(1))]_j
        $$
