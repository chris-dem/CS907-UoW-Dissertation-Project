The first objective of our disseration was to reduce the
\textit{EndOfLine} problem to the \textit{PureCircuit} problem
parsimoniously. We emphasize, that a non-parsimonious
reduction chain was already established by Deligkas et al. \cite{deligkas_PureCircuitTightInapproximability_2024}.
Due to the difficulty of achieving such reductions, we tried to tackle the problem from both
directions. One is by finding related problems from the \textit{EndOfLine}
and the other is by finding problems that reduce to the \textit{PureCircuit} problem.
More specifically, we list the following theorems:
\begin{enumerate}
...
\end{enumerate}
We define the above problems and the theorems in the upcoming sections and
argue that the above results bring us closer to a parsimonious reduction from the \textit{EndOfLine} problem.
...
we focus on \textit{Discrete panchromatic} instances, where there are no
consecutive panchormatic cubes, and define its complexity class as \ref{def:hf-discrete-strong-sperner}.
...
\begin{definitionbox}{\textit{HF-DiscreteStrongSperner} problem}{hf-discrete-strong-sperner}
    \label{def:hf-discrete-strong-sperner}
...
\begin{proof}
    ...
    \end{enumerate}
    The 1st and last part of our construction is self-explanatory, we explain the \textit{bit generator gadget} in greater depth
    ...
    that every value node has an in-degree of at most $1$.
    ...
    By our copy that, we know that $\mathbf{x}[u^i]= \mathbf{x}[s_i]$.
...
We know that \textit{EndOfLine} can parsimoniously reduce to the \textit{2D-StrongSperner} problem using
planar embedding method by Chen et al. \cite{chen_Complexity2DDiscrete_2009},
...
We will then demonstrate how we can apply with the \textit{Bipolar colouring}, where we extend
the colouring technique Chen et al. \cite{chen_Complexity2DDiscrete_2009}.
...
or the square that contains $e_{P_{i}}$ and the end of a line will be panchromatic.
...
\begin{proof}
    ...
    \begin{definitionbox}{Snake Embedding operation $S(\Lambda^t, i)$}{snake-emb-op}
        ...
        $\Lambda^{t+1}: M^{t+1} \to \{-1, +1\}^{n+1}$ such that $m'_i = s + 3, n_{t+1} = n_t +1, m_{n_{t+1}} = 8$, where $\Lambda^{t+1}$ is defined
    ...
    \begin{proof}
        ...
        WLOG assume that $(\cdot, j+1)$  corresponds to the point $(\cdot, p_j + 1, p_x)$.
        We can observe that the above correspondance occurs on the horizontal portions. On
        the vertical portions, the $(\cdot, j+1)$ point corresponds to $(\cdot, p_j, p_x+1)$.
        We can make the same arguments for any dimensionality permutation so we avoid analysing every scenario.
        That would imply that
        points $(\cdot, p_j, p_x), (\cdot, p_j + 1, p_x)$ yield a panchromatic solution for the $n_t$-dimensional subspace.
        In order to cover the $n_{t+1}$-th dimension, we can observe that
        ...
        We can also observe that if the effective dimension is on the $j$ dimension then if $p$ is a panchromatic anchor of the new subspace:
        $$
            \forall b \in \{0,1\}^{n_{t}}, j \in [n_{t+1}]:  [\lambda(p + b0)]_j = - [\lambda(p  + \bar{p}1)]_j
        $$
    ...
    of such circuit will take applications of each operation $O(n\log_32)$. This concludes our proof.
\end{proof}
...
and \cite{deligkas_PureCircuitTightInapproximability_2024}, it is commont to take advantage of speculative
...
between the solutions of the original problem domain and the \textit{PureCircuit} domain.
