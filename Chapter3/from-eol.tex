...
\begin{proof}
    ...
    \end{proof}
    To ensure that $\Lambda$ is hazard free we can use the colorrary \ref{col:k-bit-haz} where we can create an
    $n$-bit hazard-free pure circuit. One must observe that even though, the size of the circuit increases exponentially
    with respect to the dimensions, since the problem scales with width of the space $k$ and we assume $k \gg n$, the size will scale
    up to a constant factor with respect to the input size.
...
\begin{proof}
    ...
    First, we define mapping $\mathcal{F}: V_{2^k} \to B_{2^{2k+5}}$ such that %TODO: Careful about edges
    $$
