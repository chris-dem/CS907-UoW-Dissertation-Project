...
We built upon the \textit{StrongSperner} idea that Deligkas
used by expressing how we can redefine the problems under \textit{Kleene logic} \cite{deligkas_PureCircuitTightInapproximability_2024}.
...
We essentially claim that a panchromatic solution of a \textit{HF-AntipodalStrongSperner}
contains all $2^d$ colours.
...
\subsubsection{Hazard Free StrongSperner To PureCircuit}
...
\begin{proof}
    ...
    By our copy that, we know that $\mathbf{x}[o^i]= \mathbf{x}[s_i]$.
...
If we try to use the corollary 10 by \ref{col:k-bit-haz},
