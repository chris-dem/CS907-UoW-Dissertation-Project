...
ask the question: does the functon $h(G) \triangleq f(G)/ 6$ also count something?
...
We say that this is a ``combinatiorial interpretation of $h(G)$''.
...
So now we can ask the question: does the function $e'(C) \doteq e(C) -1$ also
have a set of object which we can enumerated over?
Ikenmeyer et al. \cite{ikenmeyer_WhatWhatNot_2022} demonstrated that
this problem does not have a combinatorial interpretation.
...
In fact, being able find such definitions or interpretations is crucial,
...
at the subclasses of $\textbf{\#TFNP}-1$ problems, a class of problems
whose combinatorial nature guarantee the existence of a solution.
More specifically, we look into the \textbf{PPAD} class which
seems to have several problems where under decrementation,
may or may not have a combinatorial interpretation \cite{ikenmeyer_WhatWhatNot_2022}.
This implies that \textbf{PPAD} acts as some sort of barrier,
to $\textbf{\#P}$ which modivates the research as to what causes

To accomplish this task,
we studied \textit{PureCircuit}
...
directions that we uncovered. Morevover, we will give an overview of the visualisation software
